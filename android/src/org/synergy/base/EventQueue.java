...
public class EventQueue implements EventQueueInterface {
    ...
    private Set <EventQueueTimer> timers;
    private PriorityQueue <EventQueueTimer> timerQueue;
    ...
    // public newTimer (double duration, Object target) {
    // TODO Timer stuff
    // }

//    public void newOneShotTimer ();
//    public void deleteTimer ()

    /*
    public synchronized void adoptHandler (Integer type, Object target, EventJobInterface handler) {
        // set/replace current handler
    	Map <Integer, EventJobInterface> handlerMap = handlers.get (target);
    	
    	if (handlerMap == null) {
    		// First handler for this event type
    		handlerMap = new HashMap <Integer, EventJobInterface> ();
    ...
    */
    ...
    /*
    private boolean hasTimerExpired (Event event) {
        // return true if there's a timer in the timer priority queue
        //  that has expired.  if returning true then fill in event appropriately
        ...
        for (EventQueueTimer iterator : timerQueue) {
            iterator.subtractTime (elapsedTime);
        }

        // done if no timers are expired
        if (timerQueue.peek().getTime () > 0.0) {
            return false;
        }

        // remove timer from queue
        EventQueueTimer removedTimer = timerQueue.remove ();

        // prepare event and reset the timer's clock
        removedTimer.fillEvent (timerEvent);
        event = new Event (EventType.TIMER, removedTimer.getTarget (), timerEvent);
        removedTimer.reset ();

        // reinsert the timer into queue if it's not a one-shot
        if (!removedTimer.isOneShot ()) {
            timerQueue.add (removedTimer);
        }
...
//    private Integer saveEvent (final Event event);

    // private Event removeEvent (Integer eventID);
